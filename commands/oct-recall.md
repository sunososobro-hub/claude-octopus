...
Read ONLY the index file at `~/.claude/projects/<project-slug>/memory/MEMORY.md`,
where `<project-slug>` is Claude Code's own project directory for the current
working directory (same convention as its built-in auto-memory feature — the
cwd with `/` replaced by `-`, e.g. `/home/alonso` → `-home-alonso`). If unsure
of the exact slug, list `~/.claude/projects/` and match the entry whose
`memory/MEMORY.md` exists for the current project.
